...
|---|---|---|---|
| BUG-020 | Alto | T-02 | **Em Progresso** — lote 1/booking mergeado (PR #8, 2 IDORs fechados); lotes restantes (partners, assessments, journey, queries) mantêm o item aberto |
...
- Execução: Em andamento — **~5,5/11 (~50%)** (ver `DASHBOARD.md`). BUG-020 é
  sistêmico e feito em lotes: **lote 1 (booking) mergeado (PR #8)** — conta como
  meio no numerador (bug inteiro segue Em Progresso, mesma contabilidade
  fracionária do BUG-018/T-03), não como unidade fechada.
- Resultado: ✓ Corrigidos/mergeados: BUG-003 (recover sem auth, PR #3), BUG-007
  ...
  PR #5). ◐ Parcial: BUG-020 — lote 1/booking mergeado (PR #8): 2 IDORs fechados
  (`cancelBookingAction`/`submitEvaluationAction`) + `bookEventAction` com guard
  condicional que preserva o funil de lead; lotes restantes (partners,
  assessments, journey, queries, etc.) abertos. ○ Restantes: BUG-004 (vazamento
  de path, precisa avaliação de exposição), BUG-005, BUG-006, BUG-021, BUG-025
  (webhook HMAC).
...
| BUG-020 | Alto | Em Progresso (lote 1/booking mergeado, PR #8) | T-02 |
