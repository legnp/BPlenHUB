...
|---|---|---|---|
| BUG-020 | Alto | T-02 | **Em Progresso** — lote 1/booking (PR #8, 2 IDORs) + lote 2/CRUD admin (PR #9) mergeados; lotes restantes (forms/surveys analytics, journey, queries, upload, auth-permissions) mantêm o item aberto |
...
- Execução: Em andamento — **~5,7/11 (~52%)** (ver `DASHBOARD.md`). BUG-020 é
  sistêmico e feito em lotes: **lote 1 (booking, PR #8)** e **lote 2 (CRUD admin,
  PR #9)** mergeados — contam fracionado no numerador (bug inteiro segue Em
...
- Resultado: ✓ Corrigidos/mergeados: BUG-003 (recover sem auth, PR #3), BUG-007
  ...
  `requireAdmin()` em `partners.ts` + `admin-assessments.ts`. Lotes restantes
  (forms/surveys analytics, journey, queries, upload, `auth-permissions`)
  abertos. ○ Restantes: BUG-004 (vazamento de path, precisa avaliação de
  exposição), BUG-005, BUG-006, BUG-021, BUG-025 (webhook HMAC).
...
| BUG-020 | Alto | Em Progresso (lotes 1/booking PR #8 + 2/CRUD admin PR #9 mergeados) | T-02 |
