...
`DASHBOARD.md`) e a Track **T-02 (Segurança sistemática) está em 10/12 (~83%)**
...
HMAC, PR #16) fechou o último item financeiro do track; e `BUG-004` (path de
debug no lugar do apelido no painel admin, PR #17, rebaixado Alto→Baixo após
avaliação de exposição) foi corrigido. **Nenhum bug Crítico está aberto no
momento.**
...
| Segurança | Track adicional "Segurança sistemática" (matriz de guards do Mapa 4) — **T-02 em 10/12 (~83%)**: corrigidos `BUG-003/004/007/019/020/021/023/024/025/032` (inclui o item sistêmico `BUG-020`, fechado em 7 lotes, o Crítico `BUG-032`, o webhook MP com HMAC `BUG-025`, e o path de debug `BUG-004`); abertos `BUG-005/006` |
...
- Decisão: Decidida — padrão canônico de guard consolidado na prática ao longo
  ...
  `RETROSPECTIVE.md`
- Execução: Em andamento — **10/12 (~83%)** (ver `DASHBOARD.md`). **BUG-020 Corrigido**
  ...
  após avaliação de exposição — admin-only). Nenhum Crítico aberto.
- Resultado: ✓ Corrigidos/mergeados: BUG-003 (recover sem auth, PR #3), BUG-007
  ...
  **BUG-032** (escalação de privilégio no login, PR #14), **BUG-025** (webhook
  ...
  debug exposto no painel admin, PR #17). ○ Restantes: BUG-005, BUG-006
