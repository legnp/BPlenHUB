...
|---|---|---|---|
| BUG-020 | Alto | T-02 | Sistêmico (dezenas de Server Actions sem guard) — esforço em lotes, maior item aberto de segurança |
...
- Execução: Em andamento — **5/11 mergeados (~45%)** (ver `DASHBOARD.md`;
  denominador ajustado de 10 para 11 nesta reconciliação ao linkar `BUG-004`,
  que estava sem item/track vinculado)
- Resultado: ✓ Corrigidos/mergeados: BUG-003 (recover sem auth, PR #3), BUG-007
  ...
  PR #5). ○ Restantes: BUG-004 (vazamento de path, precisa avaliação de
  exposição), BUG-005, BUG-006, BUG-020 (esforço sistêmico em lotes), BUG-021,
