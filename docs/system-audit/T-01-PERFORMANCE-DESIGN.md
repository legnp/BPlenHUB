...
| **T1-3 — lista de usuários admin (F) + índices** | **Concluída — PR #160 (`257a65e`).** Medição: a lista de usuários admin (~25k leituras/abertura a 10k) faz busca substring client-side sobre a base inteira e **não tem filtro para reduzir no banco** → paginar regrediria a busca do admin; **fica para o Momento 2** (índice de busca externo, mesma razão do `Networking_Directory`). A parte de **índices** foi entregue: `firestore.indexes.json` versionado (enumerado o estado real antes — 0 compostos, só `Surveys.status` customizado, preservado) + referência no `firebase.json`, criando os índices de collection group que faltavam para **BUG-114** (`Surveys(surveyId,status)`, `Forms.formId`) e **BUG-115** (`User_Permissions.admin`). **Deploy dos índices é manual** (`firebase deploy --only firestore:indexes` ou links do console) — os bugs só fecham após o deploy (Lição 31). |

**Momento 1 concluído (2026-07-23):** T1-0 (medição), T1-1 (networking, PR #158), T1-2 (agregados admin → snapshot diário, PR #159), T1-3 (índices, PR #160; lista de usuários → Momento 2). Pendências operacionais: deploy manual dos índices (fecha BUG-114/115); follow-ups de UI do T1-2 (botão "Recalcular" + selo "atualizado em"). **Momento 2** (futuro, pós-Blaze / na escala): `Networking_Directory` + paginação da lista de usuários admin com busca server-side (índice externo); Blaze; provedores externos.
