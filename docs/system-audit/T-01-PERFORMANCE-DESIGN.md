...
| **T1-2 — agregados admin (B–E)** | **Concluída — PR #159 (`1efadc6`), deploy de produção confirmado (2026-07-23).** Medição revelou que contadores nativos exigiriam ~6-10 índices de collection group inexistentes (sem pipeline de deploy) → quebrariam o painel. Adotado **snapshot diário** (opção 3.3): novo `src/lib/admin/metrics-snapshot.ts` computa os agregados 1×/dia no cron (varredura sem filtro, sem índice novo) e grava `Admin_Metrics_Daily/{latest,dateKey}`; as 4 telas (`admin-fs`/`admin-surveys`/`admin-forms`/`admin-social-feedback`) leem 1 documento, com fallback de cálculo ao vivo antes do 1º cron. Snapshot isolado/best-effort no cron (Lição 40), compartilha o slot (sem mudança em `vercel.json`). Série histórica adianta a infra do EXP-01. Paridade validada (todos os números idênticos). Achado colateral: `getFSItemDetails` já quebrado por índice faltante = **BUG-114** (à parte). **Follow-ups:** botão "Recalcular agora" + indicador "atualizado em" (UI admin). |
| **T1-3 — lista de usuários admin (F) + índices** | **Concluída — PR #160 (`257a65e`).** Medição: a lista de usuários admin (~25k leituras/abertura a 10k) faz busca substring client-side sobre a base inteira e **não tem filtro para reduzir no banco** → paginar regrediria a busca do admin; **fica para o Momento 2** (índice de busca externo, mesma razão do `Networking_Directory`). A parte de **índices** foi entregue: `firestore.indexes.json` versionado (enumerado o estado real antes — 0 compostos, só `Surveys.status` customizado, preservado) + referência no `firebase.json`, criando os índices de collection group que faltavam para **BUG-114** (`Surveys(surveyId,status)`, `Forms.formId`) e **BUG-115** (`User_Permissions.admin`). |
| **Deploy manual dos índices** | **Concluído — 2026-07-24.** O SA do Admin SDK não tem permissão de índice e o `firebase login` local expirou (interativo, não roda na shell) — deploy não é automatizável pelo agente (nota operacional permanente: sempre passo manual da Gestora). A Gestora criou os 3 índices no console do Firebase (o composto `Surveys(surveyId,status)` pelo link direto; os 2 field overrides via "Adicionar isenção" guiada). Verificado por sonda read-only: as 3 queries que davam `FAILED_PRECONDITION` voltaram a funcionar. **`BUG-114`/`BUG-115` corrigidos.** |
| **Validação da Gestora em produção** | **Concluída — 2026-07-28.** Snapshot do T1-2 confirmado rodando (cron gerando `Admin_Metrics_Daily` diariamente, 2026-07-24 a 07-28) e validado visualmente nas 4 telas de analytics do admin. |

**Momento 1 concluído (2026-07-23) e sem nenhum resíduo (2026-07-28):** T1-0 (medição), T1-1 (networking, PR #158), T1-2 (agregados admin → snapshot diário, PR #159, validado em produção), T1-3 (índices, PR #160, deployados pela Gestora em 2026-07-24; lista de usuários → Momento 2). Follow-up não bloqueante: botão "Recalcular agora" + selo "atualizado em" na UI do T1-2 (mudança de UI, validação própria, sem prazo). **Momento 2** (futuro, pós-Blaze / na escala): `Networking_Directory` + paginação da lista de usuários admin com busca server-side (índice externo); Blaze; provedores externos.
