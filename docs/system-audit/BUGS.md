...
- Status: **Momento 1 CONCLUÍDO (2026-07-23) — 3 de 3 fases entregues.** Plano
  (`T-01-PERFORMANCE-DESIGN.md`) + 4 decisões aprovadas. **T1-1 (networking, hotspot A CRÍTICO) —
  PR #158**, **T1-2 (agregados admin B–E → snapshot diário `Admin_Metrics_Daily`) — PR #159** e **T1-3
  (índices de collection group versionados) — PR #160**, todos mergeados. A **paginação da lista de
  usuários admin** (`users-admin.ts`) foi **remetida ao Momento 2** por decisão da Gestora: a tela faz
  busca substring client-side sobre a base inteira e não há filtro para reduzir no banco; paginar sem
  índice de busca externo regrediria a busca do admin. **Pendência operacional:** deploy manual dos
  índices (`firebase deploy --only firestore:indexes`) fecha BUG-114/115.
- Decisão de execução: Precisa plano+aprovação (mudança de padrão de acesso a dado em produção) —
  **atendido** (plano aprovado; cada fase foi branch+PR próprio, deploy confirmado). **Momento 2**
  (futuro, pós-Blaze/na escala): `Networking_Directory` + paginação da lista de usuários com busca
  server-side; Blaze; provedores externos (Algolia/BigQuery).
- Commit/PR: **PR #158 (T1-1)** `45cf291`, **PR #159 (T1-2)** `1efadc6`, **PR #160 (T1-3 índices)**
  `257a65e`. Lista de usuários (paginação): Momento 2.
